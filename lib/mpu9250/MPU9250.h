...
public:
    ...
    bool available() { //checks Interrupt register on new data
        return has_connected && (read_byte(mpu_i2c_addr, INT_STATUS) & 0x01);
        
    }
    // sacha - simple loop tracking for statistics
    long lastMillis = 0;
    long readings = 0;
    long fusion = 0;
    ...
    bool update() {
        ...
        //sascha only update on interrupt
...
*/
        ...
        if(currentMillis - lastMillis > 10000){
            Serial.print("[DBG] Sensor Readings/sec: ");
            Serial.println(readings/10);
            Serial.print("[DBG] Fusions/sec:");
            Serial.println(fusion/10);
      ...
      // Sascha - mahony updated as to post 19 https://forum.arduino.cc/t/fatal-bug-in-sparkfun-and-kris-winers-mpu9250-code/619810/18
      // quat_filter.update(a[1], a[0], -a[2], g[1] * DEG_TO_RAD, g[0] * DEG_TO_RAD, -g[2] * DEG_TO_RAD, m[0], m[1], m[2], q);
